...
| 功能 | 说明 |
|------|------|
| 四套预热档位 | 标准 6 分 → 加强 8 分 → 长效 10 分 → 最长 15 分，失败可逐级加长 |
| 三套 CPU 渲染预设 | 社区推荐 / 全软件栈 / ANGLE WARP，最长预热仍失败可切换重试 |
...
| 环境面板 | 版本、路径、进程、UI 状态、渲染模式、渲染/预热预设、就绪检查 |
...
1. 完全退出微信
2. 选 **10** 切换到下一档预热（`standard → extended → long → maximum` 循环）
3. 再次执行引导（菜单 **1** 或 **10**）
4. 若已为最长 15 分钟预热仍失败，选 **9** 切换渲染预设后重试
...
| 预设 ID | 面板名称 | 秒数 | 适用场景 |
|---------|----------|------|----------|
| `standard` | 标准 6 分钟（默认） | 360 | 首次尝试，多数环境足够 |
| `extended` | 加强 8 分钟 | 480 | 标准预热后 UI 仍未暴露 |
| `long` | 长效 10 分钟 | 600 | 慢速机器或首次登录较慢 |
| `maximum` | 最长 15 分钟 | 900 | 仍失败前的最后预热档位 |

菜单 **5** 可输入**任意自定义秒数**（如 `120`、`900`）；菜单 **10** 按推荐顺序循环切换档位并立即重试引导。

命令行同样支持自定义时长：

```powershell
WeChatUIA-Tool.exe bootstrap --launch-delay 480   # 任意秒数
```
...
- **渲染预设** — 下次启动将使用的 CPU 渲染组合
- **预热档位** — 引导前模拟讲述人的时长（菜单 10 逐级加长）
- **就绪检查** — 启动前环境是否满足条件
...
5. 失败后先选 **10** 加长预热；最长预热仍失败再选 **9** 换渲染预设
...
| UI 一直 `Qt51514QWindowIcon` | GPU 渲染或预热不足 | 菜单 10 加长预热；仍失败再菜单 9 换预设 |
